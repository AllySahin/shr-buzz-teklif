...
export default function UserPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const deleted = saved.deletedProductKeys ?? [];
      setDeletedProductKeys(deleted);
      // Varsayılan ürünlerden aktif veya arşivde veya kalıcı silinmişlerde olmayanları arşive ekle
      ...
      const missing = defaultProducts.filter(
        (dp) => {
          const key = `${dp.urunAdi}|${dp.ebat}`;
          return !activeKeys.has(key) && !archivedKeys.has(key) && !deletedKeys.has(key);
        }
      );
  ...
  useEffect(() => {
    if (!loaded) return;
    saveFormData({ firmaAdi, teklifNo, products, archivedProducts, deletedProductKeys, bilgilendirmeSatirlari });
  }, [firmaAdi, teklifNo, products, archivedProducts, deletedProductKeys, bilgilendirmeSatirlari, loaded]);
  ...
  const deleteArchivedProduct = (archiveIndex: number) => {
    const product = archivedProducts[archiveIndex];
    setDeletedProductKeys((prev) => [...prev, `${product.urunAdi}|${product.ebat}`]);
    setArchivedProducts((prev) => prev.filter((_, i) => i !== archiveIndex));
  };
          ...
          {showArchive && (
            <>
              {archivedProducts.length === 0 ? (
                <div className="px-4 sm:px-6 py-8 border-t border-slate-200 text-center">
                  <p className="text-sm text-slate-400">Arşivde ürün bulunmuyor.</p>
                </div>
              ) : (
                <>
